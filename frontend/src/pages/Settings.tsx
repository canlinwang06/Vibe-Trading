...
export function Settings() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.error(`模型设置保存失败：${error instanceof Error ? error.message : "未知错误"}`);
    } finally {
      setSaving(false);
    }
  };
  ...
  const submitDataSources = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.error(`数据源设置保存失败：${error instanceof Error ? error.message : "未知错误"}`);
    } finally {
      setDataSaving(false);
    }
  };
  ...
  const localApiAccessSection = (
    <form onSubmit={submitLocalApiKey} className="rounded-lg border bg-card p-5 shadow-sm">
      <div className="mb-4 space-y-1">
        ...
        <p className="text-sm text-muted-foreground">{"远程或私有 Web UI 部署时，在此浏览器保存一次服务端 API 密钥。本机使用可以留空。"}</p>
      </div>
      ...
      <p className="mt-2 text-xs text-muted-foreground">{"仅保存在当前浏览器。留空可清除。"}</p>
    </form>
  );
    ...
    return (
      <div className="mx-auto max-w-5xl space-y-6 p-6">
        <div className="space-y-2">
          ...
          <p className="max-w-3xl text-sm text-muted-foreground">{"配置本地项目的模型接入和 A 股数据源凭据。"}</p>
        </div>
  ...
  const keyStatus = settings.api_key_configured
    ...
    : settings.api_key_required
      ...
      : selectedProvider?.auth_type === "oauth" && selectedProvider.login_command
        ? `该提供商使用 OAuth。运行：${selectedProvider.login_command}`
        : "该提供商不需要 API key。";
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      ...
      <div className="space-y-2">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">{"选择研究智能体使用的模型，并保存到项目本地 agent/.env 文件。"}</p>
      </div>
        ...
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          ...
          <div className="grid gap-4">
            <label className="grid gap-2">
              ...
              <span className={hintClass}>{"切换提供商会同步更新推荐模型和端点。"}</span>
            </label>
            ...
            <label className="grid gap-2">
              ...
              <span className={hintClass}>{"请填写提供商要求的完整模型 ID。"}</span>
            </label>
        ...
        <section className="rounded-lg border bg-card p-5 shadow-sm">
          ...
          <div className="grid gap-4">
            ...
            <label className="grid gap-2">
              ...
              <span className={hintClass}>{"模型回答前的推理强度。越高越细致但更慢；需要最快回复时保持关闭。"}</span>
            </label>
      ...
      <form onSubmit={submitDataSources} className="rounded-lg border bg-card p-5 shadow-sm">
        <div className="mb-5 space-y-1">
          ...
          <p className="text-sm text-muted-foreground">{"配置回测和研究智能体使用的 A 股市场数据凭据。"}</p>
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)]">
          <div className="grid gap-4">
            <label className="grid gap-2">
              ...
              <div className="flex items-center justify-between gap-3">
                <span className={hintClass}>{"用于 A 股、基金和宏观数据。未设置时，项目会优先使用可用的免费 A 股数据源。"}</span>
                <label className="flex shrink-0 items-center gap-2 text-xs text-muted-foreground">
                  <input
                    type="checkbox"
