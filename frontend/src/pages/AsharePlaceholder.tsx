...
const PAGES: Record<PlaceholderKey, {
  ...
  eventRadar: {
    ...
    primary: "等待 PR-05/PR-06 接入信息源与事件抽取。",
    secondary: "当前页面仅展示产品壳，不读取外部新闻，也不会生成交易信号。",
    icon: Radar,
  },
  sectorRadar: {
    title: "板块雷达",
    ...
    primary: "等待 PR-08 接入板块热度评分。",
    secondary: "当前页面不展示实时行情，避免把占位数据误认为研究结论。",
    icon: Layers,
  },
  candidatePool: {
    title: "候选股票池",
  ...
  backtestResults: {
    ...
    primary: "等待 PR-11/PR-12 接入批量回测和结果页。",
  ...
  dataSources: {
    ...
    primary: "等待 PR-03/PR-04 接入本地数据仓库和行情主数据。",
    secondary: "Codex 模型仍使用 ChatGPT OAuth，不需要 OPENAI_API_KEY。",
    icon: Database,
  },
};
